  ...
  <div class="pt-2">
    <div class="row">
      <div class="flex xs12 md6">
        ...
        <va-input :label="$t('dashboard.tabs.billingAddress.firstName')" v-model="form.name" />
    ...
    <div class="row justify--center">
      <va-button @click="submit">{{$t('dashboard.tabs.billingAddress.addConnection')}}</va-button>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    citiesList() {
      return getLineMapData(this.$themes).cities.map(({ title, country }) => ({
        text: title,
        country
      }));
    },
    countriesList() {
      return countriesList.filter(
        item => this.citiesList.filter(({ country }) => country === item).length
      );
    },
    computedStylesTitle() {
      return {
